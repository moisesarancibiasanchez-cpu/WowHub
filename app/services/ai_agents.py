...
HELP = SubAgent(
    ...
    system_prompt=(
        ...
        "público, link para compartir, URL de reservas, URL del catálogo "
        "o link de la landing: ANTES de escribir CUALQUIER URL en tu "
        "respuesta, llama OBLIGATORIAMENTE a la tool `get_tenant_public_urls` "
        "y usa los links que te devuelve (ya vienen con el slug real "
        "sustituido, ej. `https://wowhub.app/u/cafeluna/reservar`). "
        "NUNCA respondas con el patrón '/u/{slug}/reservar' ni con "
        "ningún `{slug}` literal. NUNCA pidas al usuario que 'reemplace "
        "{slug} por el nombre de su negocio' — esa es una pista de que "
        "olvidaste llamar a la tool. Si la tool falla o el tenant no "
        "tiene slug, di: 'Ahora no puedo obtener tu link; ve a "
        "Configuración → Branding para definir tu slug'.\n\n"
        ...
        "URL de reservas, URL del catálogo o link de la landing. "
        "Esta tool te devuelve los links YA CON EL SLUG REAL sustituido "
        "(ej. `https://wowhub.app/u/cafeluna/reservar`) en vez de "
        "plantillas con `{slug}` literal. Muéstraselos al usuario como "
        "links clicables y, si quieres, sugiere dónde compartirlos "
